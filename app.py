...
def compliments_results():
    ...
    get_users_name = request.args.get('users_name')
    get_wants_compliments = request.args.get('wants_compliments')

    compliment_amount = int(request.args.get('num_compliments'))

    select_compliments = random.sample(list_of_compliments, compliment_amount)

    context = {
        'users_name': get_users_name,
        'wants_compliments': get_wants_compliments,
...
def animal_facts():
    ...
    your_animal_facts = request.args.get("animal")

    context = {
        'animal_facts': animal_to_fact,
        'your_animal_facts': your_animal_facts
    }
...
def image_filter():
    ...
    if request.method == 'POST':
        ...
        #Should work since type in methods is both
        context = {
            "filters": filter_types_dict.keys()
        }
        return render_template('image_filter.html', **context)
